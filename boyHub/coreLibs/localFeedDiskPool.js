...
LocalFeedDiskPool.prototype.set = function(key, value, options, callback) {
    ...
    boyHubCommon.writeTextDataToFile(path.join(this.localFeedPath,key+".txt"),value,function(err,result){
 
           if(result){
                // Set TTL
                if (options.ttl) {
                    setTimeout(() => {
                        boyHubCommon.deleteFile(path.join(this.localFeedPath,key+".txt"),function(result){
...
LocalFeedDiskPool.prototype.get=function(key, callback) {
    ...
    boyHubCommon.readTextDataFromFile(paht.join(this.localFeedPath,key+".txt"),function(err,data){
...
LocalFeedDiskPool.prototype.del=function(key, callback) {
    ...
    boyHubCommon.deleteFile(paht.join(this.localFeedPath,key+".txt"),function(result){
