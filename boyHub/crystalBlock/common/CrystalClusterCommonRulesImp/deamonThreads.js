...
DeamonThreads.prototype.nodePerformanceCollect=async function(httpMode){
    ...
    for(let ip in crystalCluster){
        ...
        if(boyHubCommon.whetherCrystalNodeItem(ip.trim()))   //remove non-ip key process
        {
           ...
           // if(crystalNodeIp.trim()!==boyHubCommon.getCurrentServerIpAdress().trim()
           // ||crystalNodeIp.trim()==="127.0.0.1"){
                    ...
                    let res=await BoyHubHttpHelper.apiSimpleRequest(httpMode,domainUrl,partialUrl,qs,body,timeout);

                    body={
                        'type':'osInfo'  
                        };
                    let res2=await BoyHubHttpHelper.apiSimpleRequest(httpMode,domainUrl,partialUrl,qs,body,timeout);
                ...
                this.crystalClusterInfoRecord.crystalClusterBlockSelect("","where crystalNodeIp=? and crystalNodePort=? and interactProtocolType=?",[crystalNodeIp,crystalNodePort,conf.platformArch.crystalCluster.interactProtocolType],"",function(rows){
                    ...
                    if(res!=undefined&&rows!=undefined){
                        if(rows.length>0){
                                ...
                                var crystalClusterBlock=  new CrystalClusterBlock(boyHubCommon.getUUID(),crystalNodeIp,crystalNodePort,conf.platformArch.crystalCluster.interactProtocolType
                                ,res["totalHeap"],res["usedHeap"],res["totalForCurrentProcess"],res["totalOnV8EngineUsing"],res["usedMemRate"],res["cpuArch"],JSON.stringify(res["cpuInfo"]),res["freemem"],res["hostName"]
                                ,JSON.stringify(res["loadAvg"]),JSON.stringify(res["networkInterface"]),res["platformtype"],res["platformVersion"],res["osTempDir"],res["totalMemory"],res["osType"],res["nodeNormalRunedTime"],1);
...
DeamonThreads.prototype.timelySelfNatureSelectionVote=function(){
    ...
    //Base on collected node performance info,select self preferred master node and do the vote on current nodeDB.
     this.crystalClusterInfoRecord.crystalClusterBlockSelect("","where crystalNodeId>0 and crystalNodeIp<>?",[boyHubCommon.getCurrentServerIpAdress().trim()],"",function(rows){
                        ...
                        {   
                                ...
                                crystalClusterInfoRecord.crystalMasterVoteSelect("1","where crystalNodeId=? and crystalNodeGuid=?",[masterNodeRow.crystalNodeId,masterNodeRow.crystalNodeGuid.trim()],"",function(rows2){
                                    ...
                                    return new Promise(function(resolve,reject){
                                            ...
                                            if(rows2.length>0){
                                                var crystalMasterVote=new CrystalMasterVote(rows2[0].crystalNodeId,rows2[0].crystalNodeGuid,rows2[0].lastVoteMeNodesIps.indexOf(boyHubCommon.getCurrentServerIpAdress().trim())<0? rows2[0].lastVoteMeNodesIps+";"+boyHubCommon.getCurrentServerIpAdress().trim():rows2[0].lastVoteMeNodesIps,
                                                ...
                                                var crystalMasterVote=new CrystalMasterVote(masterNodeRow.crystalNodeId,masterNodeRow.crystalNodeGuid.trim(),boyHubCommon.getCurrentServerIpAdress().trim(),
                                                ...
                                                boyHubCommon.GetFormatDateFromTimeSpan(Date.now()),masterNodeRow.crystalNodeIp,masterNodeRow.crystalNodePort,conf.platformArch.crystalCluster.interactProtocolType);
     ...
     function SelfNatureSelectionVote(rows,callback){
            ...
            if(rows.length===crystalClusterNodeCount-1){
                ...
                let min_mem_heapUsedTemp=boyHubCommon.getCurrentNodeMem().usedHeap;  
                let min_mem_usedMemRateTemp=boyHubCommon.getCurrentNodeMem().usedMemRate;
                let max_freememTemp=0;
                let min_loadAvg1Temp=boyHubCommon.getNodeOSInfo().loadAvg[0];
                let min_loadAvg5Temp=boyHubCommon.getNodeOSInfo().loadAvg[1];
                let min_loadAvg15Temp=boyHubCommon.getNodeOSInfo().loadAvg[2];
                ...
                {
                        //select one radom
                        let radomIndex= boyHubCommon.GetRandomNum(0,rows.length-1);
                        ...
                        if(rows!==undefined)
                        {
                           // console.log('current rows:',rows);
                            if(rows.length>0){
                                ...
                                boyHubCommon.GetFormatDateFromTimeSpan(Date.now()),row.crystalNodeIp,row.crystalNodePort,conf.platformArch.crystalCluster.interactProtocolType);
                                crystalClusterInfoRecord.crystalMasterVoteInsert(crystalMasterVote);
                            }
                        }
...
DeamonThreads.prototype.getCurrentNodeRole=function(callback){
    ...
    for(let ip in crystalCluster){
         ...
         if(boyHubCommon.whetherCrystalNodeItem(ip.trim()))   
         {
             let crystalNodeIp=null,crystalNodePort=null;
             if(crystalCluster[ip].split(":").length>1){
    ...
    //random select one of crystal cluster ip which not self from the set
     let randomCrystalIpPortIndex=boyHubCommon.GetRandomNum(0,crystalIpAndPortList.length-1);
    ...
    BoyHubHttpHelper.apiSimpleRequestWithCallBack(conf.platformArch.crystalCluster.httpDefaultMode,domainUrl,partialUrl,qs,body,timeout,function(res){
          //verify whether current node ip===other voted crystal cluster master node
          if(boyHubCommon.getCurrentServerIpAdress().trim()===res.crystalNodeIp.trim()){
